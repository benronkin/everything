...
.project-step .step-caption {
  color: inherit;
  text-decoration: none;
  margin-left: 10px;
}
.project-step .step-caption.completed {
  color: var(--gray3);
  text-decoration: line-through;
}
.project-step .mark-complete {
  width: 9px;
  margin-right: 15px;
  color: var(--gray6);
}
.project-step .fa-circle-check {
  color: var(--teal3);
}
.project-step .fa-close {
  color: var(--gray6);
  margin-right: 5px;
  margin-left: auto;
}
.project-step .assignee {
  margin-left: 20px;
}
`
...
function listen(el, id) {
  el.querySelector('.assignee').addEventListener('change', (e) => {
    const stepEl = e.target.closest('.project-step')
    id = id || stepEl.id
