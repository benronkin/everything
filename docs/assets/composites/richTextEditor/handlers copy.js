...
function handleArrowDown() {
  ...
  if (node.tagName === 'CODE') {
    const isLastNode = node.parentElement === getLastEditorElement()
    if (isLastNode) {
      const newNode = createEmptyNode()
      document.querySelector('.rte-editor').appendChild(newNode)
      placeCaretInside(newNode)
    }
  }
}
...
function handleEnterInCode() {
  ...
  const newLine = document.createTextNode('\n\u200B') // zero-width space to ensure height
...
function handleCode(e) {
  ...
  if (isHtml) {
    const html = editor.textContent.innerHTML
      .replace(/&lt;/gi, '<')
      .replace(/&gt;/g, '>')
      .replace(/<br>/g, '')
  ...
  popup.querySelectorAll('li').forEach((li) =>
    li.addEventListener('click', () => {
      ...
      if (replacement) {
        ...
        document.querySelector('.rte-editor').appendChild(preEl)
      }
